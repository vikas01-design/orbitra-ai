  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <h1 className="font-display text-3xl md:text-4xl font-bold text-slate-800">Skill Gap Analyzer</h1>
      ...
      <motion.form onSubmit={handleAnalyze} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <div className="relative flex-1">
          <div className="absolute inset-0 rounded-2xl bg-blue-100/60 blur-xl pointer-events-none" />
          <div className="relative neu-inset flex items-center rounded-2xl px-4 border border-blue-200">
            <Target className="w-5 h-5 text-blue-400 shrink-0 mr-3" />
            <Input
              value={targetRole}
              onChange={e => setTargetRole(e.target.value)}
              placeholder="e.g. Staff Fullstack Engineer, ML Engineer…"
              className="h-14 bg-transparent border-0 text-slate-800 placeholder:text-slate-300 focus-visible:ring-0 shadow-none text-sm"
            />
          </div>
        </div>
        <Button type="submit" disabled={runSkillGap.isPending || !targetRole.trim()}
          className="h-14 px-7 rounded-2xl font-display text-xs tracking-wider shrink-0
            ...
            text-white border-0 shadow-[0_4px_20px_rgba(37,99,235,0.3)]">
          ...
          <Modal onClose={() => setVideoPicker(null)}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-bold text-base text-slate-800">Select Language</h3>
              </div>
              <button onClick={() => setVideoPicker(null)} className="text-slate-300 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-sm text-slate-400 mb-5">For: <span className="text-blue-600 font-medium">"{videoPicker.title}"</span></p>
            <div className="grid grid-cols-2 gap-2">
              {LANGUAGES.map(lang => (
                <button key={lang.value} onClick={() => selectLanguage(lang.value)}
                  className="text-left px-3 py-2.5 rounded-xl text-sm font-medium text-slate-700
                    bg-slate-50 hover:bg-blue-50 hover:text-blue-700
                    border border-slate-200 hover:border-blue-300 transition-all">
                  {lang.label}
                </button>
              ))}
            </div>
          ...
          <Modal onClose={() => setChannelPicker(null)}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-bold text-base text-slate-800">{channelLangLabel} Channels</h3>
              </div>
              <button onClick={() => setChannelPicker(null)} className="text-slate-300 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-sm text-slate-400 mb-4">Topic: <span className="text-amber-600 font-medium">"{channelPicker.title}"</span></p>
              ...
              {channelList.map(channel => (
                <button key={channel.handle} onClick={() => openChannel(channel)}
                  className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-left
                    bg-slate-50 hover:bg-red-50 hover:text-red-700
                    border border-slate-200 hover:border-red-200 transition-all">
                  <div className="w-8 h-8 rounded-full bg-red-50 border border-red-200 flex items-center justify-center shrink-0">
                    <Youtube className="w-3.5 h-3.5 text-red-500" />
                  </div>
                  <div>
                    <p className="text-slate-800 font-semibold text-sm">{channel.name}</p>
                    <p className="text-slate-400 text-xs">{channel.handle}</p>
                  </div>
                </button>
              ))}
            </div>
            <button onClick={() => { setChannelPicker(null); setVideoPicker({ query: channelPicker.query, title: channelPicker.title }); }}
              className="mt-4 w-full text-xs text-slate-300 hover:text-slate-500 transition-colors text-center">
      ...
      <div className="space-y-4 pt-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          <h2 className="font-display text-sm font-bold text-slate-700 tracking-wider uppercase">Past Analyses</h2>
          {gaps && gaps.length > 0 && (
            <span className="text-xs text-slate-300 font-mono">{gaps.length} record{gaps.length !== 1 ? "s" : ""}</span>
          )}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="neu-card p-12 text-center">
            ...
            <p className="text-slate-400 text-sm">No analyses yet. Run one above.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {gaps?.map((gap) => (
              <div key={gap.id}
                className="neu-card overflow-hidden hover:border-blue-300 transition-all duration-300">
                <div
                ...
                >
                  <div>
                    <h3 className="font-display font-bold text-base text-slate-800">{gap.targetRole}</h3>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex gap-1.5">
                      <span className="text-xs text-emerald-600 font-mono">{gap.requiredSkills.filter(s => !gap.missingSkills.includes(s)).length} ✓</span>
                      <span className="text-xs text-rose-600 font-mono">{gap.missingSkills.length} ✗</span>
                    </div>
                ...
                <AnimatePresence>
                  {expandedId === gap.id && (
                    ...
                    >
                      <div className="px-5 pb-6 border-t border-slate-100 space-y-6 bg-slate-50/50 pt-5">

                        {/* Skills */}
                        <div className="grid md:grid-cols-2 gap-5">
                          <div className="space-y-3">
                            <h4 className="text-[10px] font-bold tracking-[0.18em] text-emerald-600 uppercase flex items-center gap-1.5">
                              <Zap className="w-3.5 h-3.5" /> Validated Skills
                            </h4>
                            <div className="flex flex-wrap gap-2">
                              {gap.requiredSkills.filter(s => !gap.missingSkills.includes(s)).map(skill => (
                                <Badge key={skill} variant="secondary" className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs">{skill}</Badge>
                              ))}
                              {gap.requiredSkills.filter(s => !gap.missingSkills.includes(s)).length === 0 && (
                                <p className="text-xs text-slate-300">None validated.</p>
                              )}
                            </div>
                          </div>
                          <div className="space-y-3">
                            <h4 className="text-[10px] font-bold tracking-[0.18em] text-rose-600 uppercase flex items-center gap-1.5">
                              <Target className="w-3.5 h-3.5" /> Missing Skills
                            </h4>
                            <div className="flex flex-wrap gap-2">
                              {gap.missingSkills.map(skill => (
                                <Badge key={skill} variant="secondary" className="bg-rose-50 text-rose-700 border border-rose-200 text-xs">{skill}</Badge>
                              ))}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 pt-4 border-t border-slate-100">
                          <h4 className="font-display font-bold text-sm text-slate-800 flex items-center gap-2">
                            <BookOpen className="w-4 h-4 text-blue-600" /> Learning Roadmap
                          </h4>
                          <div className="space-y-4 pl-4 border-l-2 border-blue-200 relative">
                            {gap.roadmap.map((step: any, i: number) => (
                              ...
                              >
                                <div className="absolute -left-[29px] top-1.5 w-3.5 h-3.5 rounded-full bg-blue-500 border-[3px] border-white shadow-[0_0_8px_rgba(37,99,235,0.4)]" />
                                <div className="flex items-start justify-between gap-3">
                                  <div className="flex-1 min-w-0">
                                    <p className="text-[10px] font-mono text-blue-500 mb-0.5">{step.day}</p>
                                    <h5 className="font-semibold text-sm text-slate-700 mb-1">{step.title}</h5>
                                  ...
                                  {step.videoQuery && (
                                    <button onClick={() => setVideoPicker({ query: step.videoQuery, title: step.title })}
                                      className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
                                        ...
                                        hover:bg-red-100 hover:border-red-300 transition-all mt-0.5">
